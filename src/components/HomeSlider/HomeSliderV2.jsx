...
const HomeSliderV2 = ({homeSliderData}) => {
    return (
        <div className='py-4'>
             <Swiper
        spaceBetween={30}
      ...
      >
        {
          homeSliderData?.length !== 0 && homeSliderData?.slice(0,4)?.map((item, index) =>{
            return (
  ...
  <div className="relative h-[240px] md:h-[440px]">
    <img
      src={item?.images?.[0] || 'https://via.placeholder.com/1200x600?text=Banner'}
      alt={item?.name || 'banner'}
      className="absolute inset-0 h-full w-full object-cover transition-transform duration-700 group-hover:scale-[1.03]"
      loading="lazy"
    />
    <img
      src={item?.images?.[1] || item?.images?.[0] || 'https://via.placeholder.com/1200x600?text=Banner'}
  ...
  <div className="absolute inset-y-0 left-0 w-[78%] md:w-[46%] translate-x-[-6%] md:translate-x-[-10%] transition-transform duration-700 group-hover:translate-x-0">
    <div className="h-full w-full rounded-r-[28px] bg-white/70 backdrop-blur-xl ring-1 ring-gray-200 shadow-[0_10px_30px_rgba(2,6,23,0.08)] px-4 md:px-8 py-5 md:py-10 [clip-path:polygon(0%_0%,100%_0%,85%_100%,0%_100%)]">
      ...
      {/* Price line */}
      <div className="mt-3 flex items-center gap-2 text-sm md:text-base font-semibold text-slate-700">
        Starting at only
        <span className="inline-flex items-center rounded-full bg-sky-50 px-2.5 py-0.5 text-sky-800 ring-1 ring-sky-200 text-[14px] md:text-[18px]">
          TK {item?.price}
        </span>
      </div>

      {/* CTA */}
      <Link to="/productListing" className="mt-4 inline-block">
  ...
  <div className="md:hidden absolute inset-x-3 bottom-3 rounded-xl bg-white/90 backdrop-blur-md p-3 ring-1 ring-gray-200 shadow-md">
    <h3 className="text-[13px] font-semibold text-slate-900 line-clamp-2">{item?.name}</h3>
    <div className="mt-1 flex items-center justify-between">
      <span className="text-[13px] font-bold text-sky-700">TK {item?.price}</span>
...
HomeSliderV2.propType={
  homeSliderData:PropTypes.object
}