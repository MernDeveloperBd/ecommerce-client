...
const CategoryGridSection = () => {
  ...
  // ObjectId/string/array — সব কেস হ্যান্ডেল
  const idStr = (id) => {
  ...
  const hasCat = (pCat, catId) => {
    if (Array.isArray(pCat)) return pCat.map(idStr).includes(idStr(catId));
    return idStr(pCat) === idStr(catId);
  };

  // Utility: get 4 products of one category
  const getCategoryProducts = (catId) =>
    products.filter((p) => hasCat(p.catId, catId)).slice(0, 4);
            ...
            <div className="grid grid-cols-2 gap-2 mb-4">
              {catProducts.map((p) => (
                <Link
                  ...
                  key={p._id}
                  className="group/img relative block overflow-hidden rounded-xl ring-1 ring-gray-200 transition-all duration-300 hover:ring-violet-300 hover:shadow-[0_8px_24px_rgba(124,58,237,0.15)]"
                >
